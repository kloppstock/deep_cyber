...
/* specialized 2D convolution with no strides and 3x3 kernels */
Tensor conv2d_3x3(Tensor X, Tensor w, Tensor b, char padding, uint32_t groups) {
  /* extract parameters */
  ...
  assert((w.a == 3 && w.b == 3) &&
         "Error: Invalid kernel size for 3x3 Conv2D!\n");

  /* same padding */
  if (padding) {
    /* calculate output dimensions and create output tensor */
    uint32_t rows_out = rows_in;
    uint32_t cols_out = cols_in;
    Tensor out =
    ...
    /* calculate output dimensions and create output tensor */
    uint32_t rows_out = rows_in - 2;
    uint32_t cols_out = cols_in - 2;
    Tensor out =
        create_tensor(batch, rows_out, cols_out, groups * channels_out);
...
Tensor conv2d(Tensor X, Tensor w, Tensor b, uint32_t stride_rows,
  ...
  /* execute specialized version if applicable */
  if (kernel_rows == 3 && kernel_cols == 3 && stride_rows == 1 &&
      stride_cols == 1)
    return conv2d_3x3(X, w, b, padding, groups);
